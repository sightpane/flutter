...
- **Contract coordinates** — which envelope item type/field (`lib/src/models.dart`
  ...
  `frontend/lib/core/config.dart`, `lib/src/options.dart`).
...
- **Existing pins** — the tests that assert the current behaviour:
  `test/*.dart` (queue, replay, pointer, widgets, hog), `backend/*_test.go`
...
- `lib/src/queue.dart:88` — `_drain`, the retry path
...
- `test/queue_test.dart` "a failed send keeps items and retries with backoff"
...
- **Three parts, one contract.** If the fix changes an envelope item or field, the issue
  covers both `lib/src/models.dart` and `backend/internal/store/store.go` (and the dashboard
...
- **Env vars are config.** Name the variable, its default, where it is read
  (`backend/main.go`, `backend/internal/store/store.go`, `frontend/lib/core/config.dart`,
  `lib/src/options.dart`), and update `docker-compose.yml` / READMEs.
