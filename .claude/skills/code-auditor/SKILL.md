---
name: code-auditor
description: Act as an expert Staff Engineer to deeply audit the Dart/Flutter SDK published as `sightpane` on pub.dev for bugs, logic errors, concurrency and lifecycle issues, code smells and anti-patterns. Trigger this when the user asks to "review my code", "find bugs", "identify code smells", "audit this", or "check for best practices". It categorizes findings by severity and proposes structured solutions.
---
...
|---|---|---|---|
| SDK | `package/` (`sightpane`) | Dart/Flutter, only `http` + `clock` + `web` deps | `flutter test` |
...
The wire contract between them is the **envelope** (`POST /api/v1/envelope`, `X-Sightpane-Key`): items of type `breadcrumb`, `event`, `error`, `frame`, `pointer`, `heartbeat`, `session_end`. A change on one side of that contract is a finding on the other side until proven compatible (`backend/api_test.go` and `test/models_test.dart` pin it).
...
6. **Known limitations are not findings.** The READMEs list deliberate limits (`README.md` "Sınırlar", `backend/README.md`, `frontend/README.md`): no persistent disk queue in the SDK, frame-based replay has no text search, single-node SQLite, `SightpaneMask` masks only the wrapped widget's rect, the last batch may be lost on web tab close. Flag one only if it has regressed or the README claim is now false.
