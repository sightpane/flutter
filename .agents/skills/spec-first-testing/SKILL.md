...
|---|---|
| The envelope contract (`lib/src/models.dart` ↔ `backend/internal/store/store.go` `itemHead`) | The function body you're about to change |
...
|---|---|---|
| SDK | `flutter test` | `test/fake_transport.dart` (`FakeTransport`), `fakeAsync` for the queue, `tester.runAsync` for `toImage`/PNG decode, `Sightpane.init(... transport: t)` |
...
- **SDK → backend.** `FakeTransport` proves the envelope was built; only a real `POST`
  proves the backend accepts it. `tool/smoke_send.dart` sends a real envelope
