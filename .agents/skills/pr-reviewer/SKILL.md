...
env -u GITHUB_TOKEN gh api repos/sightpane/flutter/pulls/M/comments --paginate
```
...
**Contract first.** `lib/src/models.dart` (what the SDK sends) ↔ `backend/internal/store/store.go`
...
(`test/fake_transport.dart`, `frontend/test/helpers/test_app.dart` `FakeApi`,
...
`repo: "sightpane/flutter"`): it anchors only lines inside diff hunks, folds the rest
